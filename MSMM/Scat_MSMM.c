...
int main(int argc, char **argv)
{
	...
	dom TF = {{wBox/2 + 100}, {wBox/2 + 100}, {hBox/2 + 100}}; // set TF domain area
	dom calc_Dom = {{-a/2, a/2}, {-a/2, a/2}, {-a/2, a/2}}; // set calc. domain
	...
	for (int n=0, N=200*wLength/calc_Wld -> dt; timer(n, N); n++) {
		updateH(calc_Wld);
		out += poyntingOut(calc_Wld, -wBox/2 - 150, wBox/2 + 150, -wBox/2 -150, wBox/2 + 150, -hBox/2 -150, hBox/2 +150);

		updateE(calc_Wld);
		out += poyntingOut(calc_Wld, -wBox/2 - 150, wBox + 150, -wBox -150, wBox + 150, -hBox/2 -150, hBox/2 +350);
		...
		if (N-n <= 2*calc_Wld -> T) {
			...
			sliceTimeAvg(calc_Wld, Ez, XZ, calc_Wld -> T, png(dkbr, -1), "/XZ-txt/");
			...
			sliceTimeAvg(calc_Wld, JE, XZ, calc_Wld -> T, png(jet, 0.02),"/XZ-JE/");
			sliceTimeAvg(calc_Wld, EE, XZ, calc_Wld -> T, txt,"/XZ-EE1/");
