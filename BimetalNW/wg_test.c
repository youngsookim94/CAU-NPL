...
int main(int argc, char **argv)
{
    ...
    putObjects(W, Drude_Ag, Ag_wire, Drude_Au, Au_wire, n(2.6), NW_wg, Air);

    pointDipole(W, Ex, sPosX, 0, sPosZ, Sine, lambda, 1, 0); //inducing fundamental mode
    ...
    { //W->N+N
        ...
        if (!(n % (W->T)))
        {
            ...
            writeRow(W, "/ratio", n * W->dt/lambda/multiplier, out / totalOut, Ref / totalOut, Trm / totalOut, positiveZ / totalOut, negativeZ / totalOut, AbsAg / totalOut, AbsAu / totalOut);
        }
