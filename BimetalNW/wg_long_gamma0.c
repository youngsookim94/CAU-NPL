...
int main(int argc, char **argv)
{
    ...
    guidedWaveZ(W, "Guided_Mode_Profiles/Ex2", Ex, 3000, Sine, lambda, 100, 1, 0);
    guidedWaveZ(W, "Guided_Mode_Profiles/Ez2", Ey, 3000, Sine, lambda, 100, 1, 0);
    ...
    for (int n = 1, N = nLoop*lambda/W->dt; timer(n, W->N+N); n++) {//W->N+N
		...
		AgOut2 = fabs(poyntingZ(W, -400, -wx/2, wx/2, -wx/2, wx/2)); 
		AgOut3 = fabs(poyntingZ(W, -800, -wx/2, wx/2, -wx/2, wx/2)); 
		AgOut4 = fabs(poyntingZ(W, -1600, -wx/2, wx/2, -wx/2, wx/2)); 
		...
		if (!(n%(W->T))) {
			writeRow(W, "/Ratio", W->dt*n/nLoop, (AuOut), (AgOut), (AgOut2), (AgOut3), (AgOut4));
			AgOut = 0;
            AgOut2 = 0;
            AgOut3 = 0;
            AgOut4 = 0;
