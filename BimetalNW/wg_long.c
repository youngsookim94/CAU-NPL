...
int main(int argc, char **argv)
{
    ...
    guidedWaveZ(W, "Ex2", Ey, sourceY, Sine, lambda, 100, 1, 0);
    guidedWaveZ(W, "Ez2", Ex, sourceY, Sine, lambda, 100, 1, 0);
    ...
    for (int n = 1, N = 500*lambda/W->dt; timer(n, W->N+N); n++) {//W->N+N
        updateH(W);
        ...
        writeRow(W, "/Totaltime", W->dt*n, get(W, Ex, sPos, sourceY ,0));
        
        if (n > W->N) {
            writeRow(W, "/ExMode", W->dt*n, get(W, Ex, sPos, sourceY, 0));
            writeSpectrum(W, N, 100, 4500, "/Spectrum", get(W, Ex, sPos, sourceY, 0));
